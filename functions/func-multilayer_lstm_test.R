...
run_multiinput_lstm_test=function(Y,indice,lag, except = 4, batch_size = 30, unit_n = 32){
  horizon = lag
  Y2 = Y %>% as.data.frame()
  ...
  if (except == 2) {
    ...
    lstm_layer_1_2 <- layer_lstm(units = unit_n, return_sequences = TRUE)(lstm_layer_1_1)
    lstm_layer_1_3 <- layer_lstm(units = unit_n, return_sequences = TRUE)(input_layer_3)
    
    # Concatenation after Level-1
    concatenated_layer_1 <- layer_concatenate(c(lstm_layer_1_2, lstm_layer_1_3))
    
    # Level-2 LSTM layer
    lstm_layer_2_1 <- layer_lstm(units = unit_n, return_sequences = TRUE)(input_layer_4)
    lstm_layer_2 <- layer_lstm(units = unit_n, return_sequences = TRUE)(concatenated_layer_1)
    
    # Concatenation for Level-2 and Level-3
    concatenated_layer_2 <- layer_concatenate(c(lstm_layer_2, lstm_layer_2_1))
    
    # Level-3 LSTM layer
    lstm_layer_3 <- layer_lstm(units = unit_n)(concatenated_layer_2)
    
    # Output layer
    output_layer <- layer_dense(units = 1)(lstm_layer_3)
    
    # Model
    model <- keras_model(inputs = list(input_layer_1, input_layer_3, input_layer_4), 
    ...
    history = model %>% fit(x = list(X.arr3, X.arr1, X.arr4), y, epochs = epochs,
                            batch_size = batch_size, shuffle = FALSE, verbose = 2)
    
    pred = model(list(X.out.arr3, X.out.arr1, X.out.arr4)) %>%
      denormalize(min(Y2[,indice]),max(Y2[,indice]))
  }
  
  ## Only Using Layer 1,2,4
  else if (except == 3) {
    input_layer_1 <- layer_input(shape = c(4, 31))
    ...
    lstm_layer_2_1 <- layer_lstm(units = unit_n, return_sequences = TRUE)(input_layer_4)
    lstm_layer_2 <- layer_lstm(units = unit_n, return_sequences = TRUE)(concatenated_layer_1)
    lstm_layer_3 <- layer_lstm(units = unit_n, return_sequences = TRUE)(lstm_layer_2)
    
    # Concatenation for Level-2 and Level-3
    concatenated_layer_2 <- layer_concatenate(c(lstm_layer_3, lstm_layer_2_1))
    
    # Level-3 LSTM layer
    lstm_layer_4 <- layer_lstm(units = unit_n)(concatenated_layer_2)
    
    # Output layer
    output_layer <- layer_dense(units = 1)(lstm_layer_4)
    
    # Model
    model <- keras_model(inputs = list(input_layer_1, input_layer_2, input_layer_4), 
                         outputs = output_layer)
    ...
    history = model %>% fit(x = list(X.arr3, X.arr2, X.arr4), y, epochs = epochs,
                            batch_size = batch_size, shuffle = FALSE, verbose = 2)
    
    pred = model(list(X.out.arr3, X.out.arr2, X.out.arr4)) %>%
      denormalize(min(Y2[,indice]),max(Y2[,indice]))
  }
  
  ## Only USing Layer 1,4
  else if (except == 4) {
    input_layer_1 <- layer_input(shape = c(4, 31))
    input_layer_4 <- layer_input(shape = c(4, 3))
    ...
    lstm_layer_1_1 <- layer_lstm(units = unit_n, return_sequences = TRUE)(input_layer_1)
    lstm_layer_1_2 <- layer_lstm(units = unit_n, return_sequences = TRUE)(lstm_layer_1_1)
    lstm_layer_1_3 <- layer_lstm(units = unit_n, return_sequences = TRUE)(lstm_layer_1_2)
    lstm_layer_2_1 <- layer_lstm(units = unit_n, return_sequences = TRUE)(input_layer_4)
    
    # Concatenation after Level-1
    concatenated_layer_1 <- layer_concatenate(c(lstm_layer_1_3, lstm_layer_2_1))
  
    # Level-3 LSTM layer
    lstm_layer_3 <- layer_lstm(units = unit_n)(concatenated_layer_1)
    
    # Output layer
    output_layer <- layer_dense(units = 1)(lstm_layer_3)
    
    # Model
    model <- keras_model(inputs = list(input_layer_1, input_layer_4), 
                         outputs = output_layer)
    ...
    history = model %>% fit(x = list(X.arr3, X.arr4), y, epochs = epochs,
                            batch_size = batch_size, shuffle = FALSE, verbose = 2)
    
    pred = model(list(X.out.arr3, X.out.arr4)) %>%
      denormalize(min(Y2[,indice]),max(Y2[,indice]))
  }
  
  ## Only USing Layer 2, 3, 4
  else if (except == 5) {
    input_layer_2 <- layer_input(shape = c(4, 9))
    input_layer_3 <- layer_input(shape = c(4, 11))
    input_layer_4 <- layer_input(shape = c(4, 3))
    ...
    # Level-1 LSTM layers
    lstm_layer_1_1 <- layer_lstm(units = unit_n, return_sequences = TRUE)(input_layer_2)
    lstm_layer_1_2 <- layer_lstm(units = unit_n, return_sequences = TRUE)(input_layer_3)
    ...
    # Level-3 LSTM layer
    lstm_layer_2 <- layer_lstm(units = unit_n, return_sequences = TRUE)(concatenated_layer_1)
    lstm_layer_2_2 <- layer_lstm(units = unit_n, return_sequences = TRUE)(input_layer_4)
    
    # Concatenation after Level-1
    concatenated_layer_2 <- layer_concatenate(c(lstm_layer_2, lstm_layer_2_2))
    lstm_layer_3 <- layer_lstm(units = unit_n)(concatenated_layer_2)
    ...
    # Model
    model <- keras_model(inputs = list(input_layer_2, input_layer_3, input_layer_4), 
                         outputs = output_layer)
    ...
    history = model %>% fit(x = list(X.arr2, X.arr1, X.arr4), y, epochs = epochs,
                            batch_size = batch_size, shuffle = FALSE, verbose = 2)
    
    pred = model(list(X.out.arr2, X.out.arr1, X.out.arr4)) %>%
      denormalize(min(Y2[,indice]),max(Y2[,indice]))
  }
...
rolling.window.lstm.minput.test=function(Y, npred, except=4, indice=1, lag=1, batch = 30, unit = 32){
  ...
  for(i in npred:horizon){
    ...
    lstm=run_multiinput_lstm_test(Y.window,indice,lag,except,batch,unit)
